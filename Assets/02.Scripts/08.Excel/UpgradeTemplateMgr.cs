...
public class UpgradeTemplateMgr : MUIBase<UpgradeTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public UpgradeTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    UpgradeTemplate pTemplate = new UpgradeTemplate(listDetail);
    ...
    public UpgradeTemplate GetTemplate(int _Index)
    {
        List<UpgradeTemplate> listTemplate = GetTList();
